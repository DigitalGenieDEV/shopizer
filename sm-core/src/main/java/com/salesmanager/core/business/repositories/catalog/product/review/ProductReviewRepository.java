...
public interface ProductReviewRepository extends JpaRepository<ProductReview, Long> {
	...
	@Query(value = "SELECT p.PRODUCT_ID \r\n"
			...
			+ "     , IFNULL(op.PRODUCT_QUANTITY, 0) AS PRODUCT_QUANTITY \r\n"
			+ "     , IFNULL(opp.PRODUCT_PRICE, 0) AS PRODUCT_PRICE \r\n"
			...
			+ "     LEFT JOIN ORDER_PRODUCT op ON p.PRODUCT_ID = op.PRODUCT_ID AND pr.ORDER_PRODUCT_ID = op.ORDER_PRODUCT_ID \r\n"
	...
	@Query(value = "SELECT p.PRODUCT_ID \r\n"
			+ "     , pr.PRODUCT_REVIEW_ID \r\n"
			+ "     , pr.DATE_CREATED AS REVIEW_DATE \r\n"
			+ "     , IFNULL(pr.REVIEWS_RATING, 0) AS REVIEWS_RATING \r\n"
			+ "     , pr.DATE_CREATED \r\n"
			+ "     , pr.DATE_MODIFIED \r\n"
			+ "     , pr.UPDT_ID \r\n"
			+ "     , prd.DESCRIPTION \r\n"
			...
			+ "     JOIN ORDER_PRODUCT_PRICE opp ON op.ORDER_PRODUCT_ID = opp.ORDER_PRODUCT_ID \r\n"
			...
			+ "      AND pd.LANGUAGE_ID = ?2 \r\n"
			+ "      AND CASE WHEN 0 = ?3 THEN pr.PRODUCT_REVIEW_ID IS NULL ELSE pr.PRODUCT_REVIEW_ID IS NOT NULL END \r\n"
		   ...
		   countQuery = "SELECT COUNT(*) \r\n"
		   		...
		   		+ "      AND CASE WHEN 0 = ?3 THEN pr.PRODUCT_REVIEW_ID IS NULL ELSE pr.PRODUCT_REVIEW_ID IS NOT NULL END \r\n"
		   		+ "ORDER BY pr.DATE_CREATED DESC \r\n", nativeQuery = true)
	Page<ReadProductReview> listByCustomerId(Long customerId, Integer languageId, Integer tabIndex, Pageable pageable);
	...
	@Query("select p from ProductReview p left join fetch p.descriptions pd join fetch p.customer pc join fetch pc.merchantStore pcm left join fetch pc.defaultLanguage pcl left join fetch pc.attributes pca left join fetch pca.customerOption pcao left join fetch pca.customerOptionValue pcav left join fetch pcao.descriptions pcaod left join fetch pcav.descriptions pcavd join fetch p.orderProduct op join fetch p.product pp left join fetch p.descriptions pd where op.id = ?1 and pc.id = ?2")
	ProductReview findByOrderProductAndCustomer(Long orderProductId, Long customerId);
