...
public class AlibabaTradeRefundReturnGoodsParam extends AbstractAPIRequest<AlibabaTradeRefundReturnGoodsResult> {
    ...
    /**
     *     物流公司编码，调用alibaba.logistics.OpQueryLogisticCompanyList.offline接口查询     *
     * 参数示例：<pre>ZTO</pre>     
     *    
     */
    ...
    /**
     *     凭证图片URLs，必须使用API alibaba.trade.uploadRefundVoucher返回的“图片域名/相对路径”，最多可上传 10 张图片 ；单张大小不超过1M；支持jpg、gif、jpeg、png、和bmp格式。 请上传凭证，以便以后续赔所需（不上传将无法理赔）     *
