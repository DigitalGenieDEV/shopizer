...
public interface PageableManagerRepository extends PagingAndSortingRepository<Manager, Long> {
	...
	@Query(value = "SELECT  A.* FROM MANAGER A WHERE  A.USER_ID  > 1 "
	  		...
	  		+ "ORDER BY DEPT_ID,POSITION_ID ASC, USER_ID DESC ",
	      countQuery = "SELECT  COUNT(USER_ID)  FROM MANAGER A WHERE  A.USER_ID  > 1	 AND ( CASE WHEN  ?1 = 'A' THEN  EMPL_ID LIKE %?2% WHEN ?1 = 'B' THEN ADMIN_NAME LIKE %?2% ELSE TRUE  END ) AND (CASE WHEN ?3  = 0 THEN DEPT_ID IS NOT NULL ELSE DEPT_ID = ?3 END)", nativeQuery=true)
