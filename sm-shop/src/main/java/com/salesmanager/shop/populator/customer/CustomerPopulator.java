			...
			if (source.getAttributes() != null) {
				for (PersistableCustomerAttribute attr : source.getAttributes()) {
					...
					if (customerOptionValue == null) {
						throw new ConversionException(
								"Customer option value id " + attr.getCustomerOptionValue().getId() + " does not exist"
						);
					}
...
//			}
			...
			if (target.getDefaultLanguage() == null) {
				...
				Language lang = source.getLanguage() == null ? language
						: languageService.getByCode(source.getLanguage());

				target.setDefaultLanguage(lang);
			}
			if (source.getCustomerTerms() != null) {
				List<CustomerTerms> values = source.getCustomerTerms().stream()
						.map(
								it -> CustomerTerms.builder()
									.id(null)
									.consented(it.isConsented())
									.customer(target)
									.expiredDate(it.getExpiredDate())
									.modifiedDate(new Date())
									.privacyCode(it.getPrivacyCode())
