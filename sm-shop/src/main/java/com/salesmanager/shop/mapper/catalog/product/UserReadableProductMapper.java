 ...
 */
@Component
public class UserReadableProductMapper  {
	...
	public ReadableProduct convert(Product source, MerchantStore store, Language language, String currencyCode) {
		ReadableProduct product = new ReadableProduct();
		return this.merge(source, product, store, language, currencyCode);
	}

	public ReadableProduct merge(Product source, ReadableProduct destination, MerchantStore store, Language language, String currencyCode) {
		...
		if (!CollectionUtils.isEmpty(source.getAttributes())) {
			List<ProductAttribute> 	attributes = productAttributeService.getByIdList(
					source.getAttributes().stream().map(ProductAttribute::getId).collect(Collectors.toList()));
		...
		System.out.println("getAttributes convert方法执行时长: " + (endTime - startTime) + " 毫秒");
		...
		try {
			FinalPrice price = pricingService.calculateProductPriceByCurrencyCode(source, currencyCode);
			if (price != null) {

				destination.setFinalPrice(pricingService.getDisplayAmount(price.getFinalPrice(), store, currencyCode));
